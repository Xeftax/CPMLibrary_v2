...
namespace cpm {
    ...
    public:
        ...
        template<typename Command, size_t ArgsNumber>
        static std::string commandRegister(std::string commandID) {
            static_assert(std::is_base_of<AbstractCommand, Command>::value, "Class must be derived from AbstractCpmCommand");
            bool result = registeredCommand.insert(std::make_pair(commandID, [](std::vector<std::string> args) -> std::shared_ptr<AbstractCommand> {
                return unpack_vector::make_shared<Command, ArgsNumber>(args);
                })).second;
